...
export const mobileCountryRegions = {
  us: [
...
export const formatHostAndPort = (input: {
  ...
  const isEuCountry = mobileCountryRegions.eu.find((rgn) => rgn.code.toLowerCase() === country);
  ...
  const isAsiaCountry = mobileCountryRegions.asia.find((rgn) => rgn.code.toLowerCase() === country);
