...
export const tlGeoV2CountryRegions = {
  us: [
...
export const formatHostAndPort = (input: {
  ...
  const isEuCountry = tlGeoV2CountryRegions.eu.find((rgn) => rgn.code.toLowerCase() === country);
  ...
  const isAsiaCountry = tlGeoV2CountryRegions.asia.find((rgn) => rgn.code.toLowerCase() === country);
