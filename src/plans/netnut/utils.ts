...
export const netNutCountryregions = {
  us: [
...
export const formatHostAndPort = (input: {
  ...
  const isEuCountry = netNutCountryregions.eu.find((rgn) => rgn.code.toLowerCase() === country);
  ...
  const isAsiaCountry = netNutCountryregions.asia.find((rgn) => rgn.code.toLowerCase() === country);
